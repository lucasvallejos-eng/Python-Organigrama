...
def CrearOrg(ORG_AUX):
    ...
    cursor.execute("CREATE TABLE Personas (COD INTEGER, DOC TEXT , APE TEXT, NOM TEXT, TEL TEXT, DIR TEXT, DEP TEXT, SAL INTEGER, Jefe INTEGER)")
...
def EspacioDependencia(Base,Tabla):
    ...
    cursor = conexion.cursor()
    Data = cursor.execute(f'SELECT  Dep5 FROM "{Tabla}"')
    dato=Data.fetchone()
    if dato[0]==None:
        bool=True
    else:
...
def CrearDep(Base,Tabla,Dep):
    ...
    while True:
        ...
        if dato[0]==None :
            ...
            cursor.execute(f'INSERT INTO "{Dep}" (COD, NOM, DepPadre, Nivel) VALUES ( ? , ? , ? , ?)',(1,Dep,Tabla,dato[5]+1 ))
            cursor.execute(f'CREATE TABLE "{DepP}" (COD REAL PRIMARY KEY, DOC TEXT , APE TEXT, NOM TEXT, TEL TEXT, DIR TEXT, DEP  TEXT, SAL REAL, Jefe INTEGER)')
...
def ModificarDepJefe(Base,Dep,CI):
    ...
    dato=Data.fetchone()
    cursor.execute(f'UPDATE Personas SET Jefe=1 WHERE COD="{dato[0]}"')
...
def IngresarPersonas(Base,Dep,persona):
    ...
    cursor=conexion.cursor()
    DepP=Dep+"P"
    cursor.execute(f'INSERT INTO "{DepP}" (COD , DOC , APE , NOM , TEL , DIR , DEP , SAL, Jefe) VALUES ( ? , ? , ? , ? , ? , ? , ? ,?, ?)',(persona.COD, persona.DOC, persona.APE, persona.NOM, persona.TEL, persona.DIR, persona.DEP, persona.SAL, '0'))
    cursor.execute(f'INSERT INTO Personas (COD , DOC , APE , NOM , TEL , DIR , DEP , SAL, Jefe) VALUES ( ? , ? , ? , ? , ? , ? , ? ,?, ?)',(persona.COD, persona.DOC, persona.APE, persona.NOM, persona.TEL, persona.DIR, persona.DEP, persona.SAL, '0'))
    conexion.commit()
    conexion.close()
...
def GraficarOrganigrama(Base, Dep):
    G = gv.Graph()
    G.graph_attr["layout"] = "dot"

    G.node_attr["shape"] = "box"
    G.node_attr["style"] = "filled"
    G.node_attr["fillcolor"] = "Sienna"
